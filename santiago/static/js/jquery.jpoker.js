...
(function($) {
	...
	// 4/17 (primative) Mobile detector
	var isMobile = {
		Android: function() {
    ...
    $.jpoker = {
        ...
        dialog_options: { width: '315px', height: 'auto', autoOpen: false, dialog: true, title: 'jpoker message'},
    ...
    var timeBankIntervalId = null;

    var timeBankIntervalId1 = null;

	var timeBeepIntervalId = null;
    //--
    ...
    jpoker.chips = {
        ...
        SHORT: function(chips) {
            ...
            return jpoker.chips.LONG(chips);
            var unit = [ 'G', 'M', 'K', '' ];
    ...
    jpoker.connection.prototype = $.extend({}, jpoker.watchable.prototype, {
			...
			pingQuery: function(obj) {
				var thisObj = obj;
				//alert(thisObj);
				thisObj.sendPacketAjax({ type: 'PacketPokerLongPollReturn' }, 'direct');
...
/*                        if (
                    ...
                    this.clearTimeout(this.incomingTimer);
                    var $this = this;
                    this.incomingTimer = window.setTimeout(function() {
                            $this.dequeueIncoming();}.bind(this),
    ...
    jpoker.server.prototype = $.extend({}, jpoker.connection.prototype, {
            ...
            getUserInfoWithCallback: function(callback) {
                this.queueRunning(function(server) {
                        server.setState(server.USER_INFO);
                        server.sendPacket({
                                type: 'PacketPokerGetUserInfo',
                                    serial: server.serial }, callback);
                    });
            },
            ...
            bankroll: function(currency_serial) {
                ...
                if(this.loggedIn() && 'money' in this.userInfo && key in this.userInfo.money) {
                    return this.userInfo.money[key][0]; // PacketPokerUserInfo for documentation
                }
                return 0;
            },
    ...
    jpoker.table = function(server, packet) {
        ...
        if (packet.seats) {
            /*J this.max_players = packet.seats; */
			this.max_players = 9;
        } else {
            this.max_players = 9; /*J*/
        }
        this.url = server.url;
        this.init();
    ...
    jpoker.player.prototype = $.extend({}, jpoker.watchable.prototype, {
            ...
            handler: function(server, table, packet) {
                ...
                case 'PacketPokerStart':
				  ...
				  $(".jpoker_auto_check input").removeAttr("checked");
				  $(".jpoker_auto_check_fold input").removeAttr("checked");
				  $(".jpoker_auto_check_call input").removeAttr("checked");
				  $(".jpoker_auto_call input").removeAttr("checked");
				  $('.jpoker_table .jpoker_player_dealt .jpoker_card').css('opacity', '1'); /*J*/
    ...
    jpoker.plugins.tableList.getHTML = function(id, packet, link_pattern) {
        ...
        for(var i = 0; i < packet.packets.length; i++) {
            ...
            if(!('game_id' in subpacket)) {
                ...
                subpacket.average_pot = subpacket.average_pot;
            }
            if (link_pattern) {
    ...
    jpoker.plugins.tourneyList.getHTML = function(id, packet, options) {
        ...
        for(var i = 0; i < packets.length; i++) {
            ...
            if(!('game_id' in subpacket)) {
                ...
                subpacket.id = subpacket.game_id + id;
                subpacket.buy_in = subpacket.buy_in;
            }
            if (t.date && (t.date !== '')) {
                        ...
                        if(packet && packet.type == 'PacketPokerTourneyManager') {
                            ...
                            if (packet.tourney.rank2prize) {
                                $.each(packet.tourney.rank2prize, function(i, prize) {
                                        packet.tourney.rank2prize[i] = packet.tourney.rank2prize[i];
                                    });
                            }
    ...
    jpoker.plugins.tourneyDetails.getHTML = function(id, packet, logged, registered, link_pattern) {
        ...
        packet.tourney.buy_in = packet.tourney.buy_in;
    ...
    jpoker.plugins.table.create = function(element, id, server, game_id) {
        ...
        if(game_id in server.tables) {
            ...
            for(var pot = 0; pot < table.pots.length; pot++) {
				if (table.pots[pot] > 0)
				{
					for (var tmp = 0; tmp < pot; tmp++ )
					{
						//$('#pots' + id + ' .jpoker_pot' + tmp).hide();
					}
				}
                jpoker.plugins.chips.update(table.pots[pot], 0, '#pots' + id + ' .jpoker_pot' + pot );
            }
            ...
            if (server.serial > 0) {
			  ...
			  $('.jpoker_ptable_leave_table').click(function() {
                  if (! confirm('Are you sure you want to leave this table?')) {
                    return;
                  }
				  ...
				  for (var i=0 ;i<9 ;i++ )
				  {
					  if ($('#seat' + i + id).hasClass('jpoker_player_self'))
					  {
						$('#sit_seat' + i + id).addClass('jpoker_ptable_chair' + i).show();
					  }
				  }
                  var server = jpoker.getServer(url);
                  var table = jpoker.getTable(url, game_id);
...
//                 element.html(jpoker.plugins.table.templates.sound.supplant({ sound: _("<span>Sound On</span>") }));
				 element.html(jpoker.plugins.table.templates.sound.supplant({ sound: _("<span></span>") }));
...
//                 element.html(jpoker.plugins.table.templates.sound.supplant({ sound: _("<span>Sound Off</span>") }));
    ...
    jpoker.plugins.table.timeout = function(id, table, serial_in_position, ratio) {
        ...
        for(var seat = 0; seat < table.seats.length; seat++) {
            ...
            if(in_position && in_position.sit_out === false && in_position.seat == seat) {
                $('.jpoker_timeout_progress', timeout_element).stop().css({width: ratio*width+'px'}).show().animate({width: '0'}, {duration: ratio*table.player_timeout*1000, queue: false, easing: 'linear'});
                timeout_element.attr('pcur', ratio*100).show();
            } else {
                timeout_element.hide();
                // action isn't on this player, so stop timer and hide timebank
                var timebank_element = $('#timebank' + id).hide();
                $('.jpoker_timebank_value', timebank_element).text('');
				$('.jpoker_ptable_timebank_bar').hide();
                clearInterval(timeBankIntervalId);
            }
	...
	jpoker.plugins.table.timeBank = function(element, timebank_remainder) {
		timeBankIntervalId1 = setInterval(timer, 1000);

		function timer(){
			if (timebank_remainder <= 0)
			{
				clearInterval(timeBankIntervalId1);
			}
			//alert(timebank_remainder);
			timebank_remainder = timebank_remainder - 1;
			element.css('width',  66.5 * timebank_remainder / 45.0 + 'px');
		}
	};
    ...
    jpoker.plugins.table.update = function(table, what, packet, id) {
        ...
        if(element && server) {
            ...
            case 'PacketPokerPlayerArrive':
                ...
                if(server.loggedIn() && packet.serial == server.serial) {
                    //J $('.jpoker_sit_seat', game_window).hide();
					for (var i = 0; i < 9; i+=1) {
						$('#sit_seat' + i + id).addClass('jpoker_ptable_chair' + i);
					}
                }
                break;
            ...
            case 'PacketPokerUserInfo':
                // They used to call playerSelf.rebuy() here, but what kind of sense
                //  does that make? Why would we want to run the rebuy function 
                //  every time we get user info? Who would expect that?
            ...
            case 'PacketPokerPotChips':
                ...
                for(var pot = 0; pot < table.pots.length; pot+=1) {
					$('#pots' + id + ' .jpoker_pot' + pot).hide();
                    if (table.pots[pot] !== 0) {
                        count += 1;
                    }
                }
                $('#pots' + id).removeClass().addClass('jpoker_ptable_pots jpoker_pots jpoker_pots'+count);
                /*J jpoker.plugins.chips.update(table.pots[packet.index], '#pots' + id + ' .jpoker_pot' + packet.index); */
				var i;
				var total = 0;
				for (i = 0; i <= count; i++){
					total = total + table.pots[i];
				}
				for (i = 0; i <= count; i++)
				{
					jpoker.plugins.chips.update(table.pots[i], total, '#pots' + id + ' .jpoker_pot' + i);
				}
                break;
            ...
            case 'PacketPokerTimeoutWarning':
                // set ratio to 0.55 instead of 0.50 to safeguard against player feeling cheated
                jpoker.plugins.table.timeout(id, table, packet.serial, 0.55);
                break;
            ...
            case 'PacketPokerTimeoutNotice':
				...
				var serial_in_position = packet.serial;
				var in_position = table.serial2player[serial_in_position];
				jpoker.plugins.table.timeout(id, table, packet.serial, 0.0);
				for(var seat = 0; seat < table.seats.length; seat++) {
					var timeout_element = $('#player_seat' + seat + id + ' .jpoker_timeout');
					if(in_position && in_position.sit_out === false && in_position.seat == seat) {
						clearInterval(timeBankIntervalId1);
						//alert(seat);
						jpoker.plugins.table.timeBank($('.jpoker_timeout_progress', timeout_element), packet.timebank_remainder);
					} else {
                ...
                if(filtered_packet !== null) {
                    ...
                    if (filtered_packet.serial === 0) {
                        ...
                        if(filtered_packet.serial in table.serial2player) {
                            ...
                            fromClass = "from_player";
                        }
                    }
					
					//Determine if scroll bar needs to be snapped
					var bSnapscroll = false;
					var container = $('.jpoker_chat_history_dealer');
					var height = container.height();
					var scrollHeight = container[0].scrollHeight;
					var st = container.scrollTop();
					
					if(st >= scrollHeight - height) //Check if box is at maximum scroll
					{
						bSnapscroll = true;
					}
					...
					for(var line = 0; line < lines.length; line++) {
                        ...
                        var chat_prefix = $('<span class=\'jpoker_chat_prefix ' + fromClass + '\'></span>').appendTo(chat_line).text(prefix);
                        var chat_message = $('<span class=\'jpoker_chat_message ' + fromClass + '\'></span>').appendTo(chat_line).text(message);
                    }
                    chat.attr('scrollTop', chat.attr('scrollHeight') || 0);
					if(bSnapscroll) $('.jpoker_chat_history_dealer').scrollTop($('.jpoker_chat_history_dealer')[0].scrollHeight /* - $('.jpoker_chat_history_dealer').height() */ );
    ...
    jpoker.plugins.table.rank = function(table, packet, id) {
        ...
        packet.money = jpoker.chips.LONG(packet.money);
    ...
    jpoker.plugins.table.templates = {
        ...
        powered_by: '',
        chat: '<div class=\'jpoker_chat_input\'><input value=\'\' type=\'text\' width=\'100%\' maxlength=\'128\' /><div class=\'jpoker_chat_enter_button\'></div></div><div class=\'jpoker_chat_history_player_box\'><div class=\'jpoker_chat_history_player_heading\'>{chat_history_player_label}</div><div class=\'jpoker_chat_history_player\'></div></div><div class=\'jpoker_chat_history_dealer_box\'><div class=\'jpoker_chat_history_dealer_heading\'>{chat_history_dealer_label}</div><div class=\'jpoker_chat_history_dealer\'></div></div>',
        wait_for_bb: '<div id="jpoker_wait_for_bb"><span>{wait_for_bb}</span><input type="checkbox" checked /> ',
        placeholder: _("connecting to table {name}"),
        table_info: '<div class=\'jpoker_table_info_name\'><span class=\'jpoker_table_info_name_label\'>{name_label}</span>{name}</div><div class=\'jpoker_table_info_separator\'></div><div class=\'jpoker_table_info_variant\'><span class=\'jpoker_table_info_variant_label\'>{variant_label}</span>{variant}</div><div class=\'jpoker_table_info_separator\'></div><div class=\'jpoker_table_info_blind\'><span class=\'jpoker_table_info_blind_label\'>{betting_structure_label}</span>{betting_structure}</div><div class=\'jpoker_table_info_separator\'></div><div class="jpoker_table_info_hand"></div><div class=\'jpoker_table_info_seats\'><span class=\'jpoker_table_info_seats_label\'>{seats_label}</span>{max_players}</div><div class=\'jpoker_table_info_flop\'>{percent_flop}<span class=\'jpoker_table_info_flop_label\'>{percent_flop_label}</span></div><div class=\'jpoker_table_info_player_timeout\'><span class=\'jpoker_table_info_player_timeout_label\'>{player_timeout_label}</span>{player_timeout}</div><div class=\'jpoker_table_info_muck_timeout\'><span class=\'jpoker_table_info_muck_timeout_label\'>{muck_timeout_label}</span>{muck_timeout}</div><div class=\'jpoker_table_info_level\'></div>',
        date: '',
    ...
    jpoker.plugins.table.callback = {
        ...
        sound: {
			...
			beep: function(server) {
				if(server.preferences.sound) {
					$('#jpokerSoundTable').html('<' + jpoker.sound + ' src=\'' + jpoker.sound_directory + 'player_timealert.swf\' />');
				}
			},
        },
        animation: {
            ...
            best_card: function(table, id, packet, duration_arg, callback) {
				...
				$('.jpoker_table .jpoker_player_dealt .jpoker_card').css('opacity', '0.5'); /*J*/
                cards_element.each(function(i) {
                        if ($(this).hasClass('jpoker_best_card') === false) {
                            if (jQuery.inArray(cards[i], packet.bestcards) != -1) {
                                /*J*/ 
								if ($(this).parent().attr('id') == 'card_seat0' + id ||$(this).parent().attr('id') == 'card_seat4' + id || $(this).parent().attr('id') == 'card_seat5' + id || $(this).parent().attr('id') == 'card_seat8' + id)
								{
									$(this).css('opacity', '1');
									$(this).css('z-index', '1'); 
									$(this).addClass('jpoker_best_card').animate({top: '-=10px'}, duration, callback);
								} else {
									$(this).css('opacity', '1');
									$(this).css('z-index', '1'); 
								}
            ...
            best_card_reset: function(table, id) {
                ...
                if (jpoker.plugins.table.callback.animation.best_card.positions !== undefined) {
                    $.each(jpoker.plugins.table.callback.animation.best_card.positions, function(id, position) {
                            $('#'+id).removeClass('jpoker_best_card').css(position).css({opacity: 1.0}).css('z-index', '0');
                        });
                }
            }
        }
    ...
    jpoker.plugins.player = {
        create: function(table, packet, id) {
			...
			//$("#player_seat"+seat+"_money"+id)
			
			$("#player_seat"+seat+id).click(function()
			{
				window.open("/tables/member/"+player.name);
			});
			///////////
            var seat_element = $('#player_seat' + seat + id);
        ...
        update: function(player, what, packet, id) {
            ...
            if(player.cards.dealt === true) {
				...
				if ($('#seat' + player.seat + id).hasClass('jpoker_player_self')) {
					$('#jpoker_wait_for_bb').hide();
				}
            } else {
                update();
			...
			setTimeout(function(){jpoker.plugins.player.handOdds(player, packet.odds, id);},200);
		    //jpoker.plugins.player.handOdds(player, packet.odds, id);
            break;
            ...
            if(server.serial == packet.serial) { 
                ...
                if (packet.type === 'PacketPokerTimeoutNotice') {
					clearInterval(timeBankIntervalId1);
                    jpoker.plugins.playerSelf.timeBank(player, packet.timebank_remainder, id);
					//server.sendPacket({ 'type': 'PacketPokerTimebank', 'serial': server.serial, 'game_id': game_id });
                }
            }
        ...
        handStart: function(player, id) {
			...
			if(table.betClampSwap > player.money) 
			{
				if(table.betClamp < player.money)
					table.betClamp = player.money;
			} else {
				table.betClamp = table.betClampSwap;
				table.betClampSwap = player.money;
			}
				
			//console.log("CHIT : " +table.betClamp);
			//console.log("CHIT : " +table.betClampSwap);
        },
        ...
        handOdds: function(player, odds, id) {
            if(jpoker.getServer(player.url).serial == player.serial) {
                setTimeout(function(){jpoker.plugins.playerSelf.handOdds(player, odds, id);},200);
            }
        },

        timeBank: function(player, timebank_remainder, id) {
            if(jpoker.getServer(player.url).serial == player.serial) {
                jpoker.plugins.playerSelf.timeBank(player, timebank_remainder, id);
            }
        },
        ...
        chips: function(player, id) {
            jpoker.plugins.chips.update(player.money, 0, '#player_seat' + player.seat + '_money' + id);
            jpoker.plugins.chips.update(player.bet, 0, '#player_seat' + player.seat + '_bet' + id);
            if(jpoker.getServer(player.url).serial == player.serial) {
        ...
        seat: function(seat, id, server, table) {
            ...
            if(table.seats[seat] !== null) {
                ...
                if(selfPlayerLoggedButNotSit && ($.inArray(seat, table.seats_left) != -1)) {
                    ...
                    $('#sit_seat' + seat + id).show();  //J Chairs appear when the user leaves table
                }
            }
        },
        ...
        side_pot: {
            ...
            update: function(player, id) {
                if (player.side_pot !== undefined) {
                    /*J var html = this.template.supplant($.extend(player.side_pot, {label: _("Pot")}));
        ...
        avatar: {
            ...
            update: function(name, url, element) {
				url = url.replace("HiResMalecut.jpg", "HiResMalecut.png"); //FORCE PNG'S FOR DEFAULT
				url = url.replace("HiResFemalecut.jpg", "HiResFemalecut.png"); //FORCE PNG'S FOR DEFAULT
        ...
        callback: {
            ...
            sound: {
				...
				beep: function(server) {
					if(server.preferences.sound) {
						$('#jpokerSoundAction').html('<' + jpoker.sound + ' src=\'' + jpoker.sound_directory + 'player_timealert.swf\' />');
					}
				}
            ...
            animation: {
                ...
                pot2money: function(player, id, packet, duration_arg, callback, hook) {
					...
					money_position.top -= pot_position.top;
					money_position.left -= pot_position.left;
    ...
    jpoker.plugins.playerSelf = {
        create: function(table, packet, id) {
			...
			$('#player_seat' + player.seat + id).unbind('click');
			
			// buy chip
			$(".jpoker_ptable_buy_chips").click(function(){
				window.open("/tables/reload/");
			});
			
			/* server.setTimeout(function() {
				server.sendPacket({ 'type': 'Packer',
									  'serial': server.serial,
									  'game_id': table.id,
									  'message': "iphone124"
									  });
            ...
            // Add initialBuyin to 'this' (i.e. jpoker.plugins.playerSelf)
            // We will invoke this from 'chips' instead of programmatically clicking rebuy 
            //  like we used to.
            this.initialBuyin = function() {
                var server = jpoker.getServer(url);
                if (server && server.loggedIn()) {
            ...
            rebuy.click(function() {
                    var server = jpoker.getServer(url);
                    if(server && server.loggedIn()) {
                        // This used to be an odd section where we called rebuy() a couple times, 
                        //  once directly and one via a side effect on getUserInfo. 
                        // Let's try making it nicer and more straight-forward...
                        var callRebuyAndOpenDialog = function() {
                            var element = jpoker.plugins.playerSelf.rebuy(url, game_id, serial, id);
                            if (element) { 
                                element.dialog('open');
                            }
                        }
                        // The problem with the following is that the callback executes before 
                        //  table/player can be updated with new user info.
                        // That kind of defeats the whole purpose of this approach.
                        // But anyway what we did was add a getUserInfo at the end of every Buy / Rebuy.
                        server.getUserInfoWithCallback(callRebuyAndOpenDialog);
                    }
                });
            ...
            var chat = function() {
                ...
                if(server) {
                    ...
                    if (message && message != '') {
					  ...
					  $('.jpoker_chat_history_dealer').scrollTop($('.jpoker_chat_history_dealer')[0].scrollHeight /* - $('.jpoker_chat_history_dealer').height() */ );

					  //$(".jpoker_chat_history_dealer").attr({ scrollTop: $(".jpoker_chat_history_dealer").attr("scrollHeight") - $(".jpoker_chat_history_dealer").height() });
                    }
                    input.attr('value', '');
                }
            };
			...
			$('.jpoker_chat_enter_button').click(function() {
				chat();
			});
			...
			// Leave Table
			//
			$('.jpoker_ptable_lobby').click(function() {
				document.location = '/tables/lobby/';
			});

			//
			// Fold to any bet
			//
			//$('.jpoker_ptable_chkbox_fold').html(jpoker.plugins.checkboxes.templates.chkbox_fold.supplant({ chkbox_fold_label: _("Fold to any bet") }));

			//
			// Sit out next hand
			//
			//$('.jpoker_ptable_chkbox_hand').html(jpoker.plugins.checkboxes.templates.chkbox_hand.supplant({ chkbox_hand_label: _("Sit out next hand") }));

			//
			// Sit out next big blind
			//
			//$('.jpoker_ptable_chkbox_blind').html(jpoker.plugins.checkboxes.templates.chkbox_blind.supplant({ chkbox_blind_label: _("Sit out next big blind") }));

			//
			// HANDMAKER
			// 
			$('.jpoker_ptable_handmaker').html(jpoker.plugins.handmaker.templates.handmakerbg.supplant({ full_house: _("FULL HOUSE"), flush: _("FLUSH"), straight: _("STRAIGHT"), kind_label: _("3 OF A KIND") }));
			if(!isMobile.any())
			{
				$('.probchart').css('border-right', '1px solid #444444');
			}
			$('.jpoker_ptable_timebank_bar').html('<div class=\'jpoker_ptable_timebank_redbar\' style=\'\'></div>');
			$('.jpoker_ptable_timebank_redbar').css('background-size', '100% 100%');
            //
            // options
            //
            ...
            // This section could use some refactoring.
            // See sendAutoMuck method for better use of constants and their evaluation (bitwise operators).
            var automuckwin = false;
            var automucklose = false;
            if (player.muck_preferences == 1 || player.muck_preferences == 3)
                automuckwin = true;
            if (player.muck_preferences == 2 || player.muck_preferences == 3)
                automucklose = true;

            var server = jpoker.getServer(url);
            $('#auto_muck_win' + id).each(function() { this.checked = /*server.preferences.auto_muck_win*/ automuckwin; });
            $('#auto_muck_lose' + id).each(function() { this.checked = /*server.preferences.auto_muck_lose*/ automucklose; });
            jpoker.plugins.muck.sendAutoMuck(server, game_id, id);
            ...
            var auto_action_element = $('#auto_action' + id).html(jpoker.plugins.playerSelf.templates.auto_action.supplant({
                ...
                auto_fold_label: _("Fold to any")
            }));

            //$('.jpoker_auto_action', auto_action_element).hide();
            $('.jpoker_auto_check').hide();
            $('.jpoker_auto_call').hide();
            $('.jpoker_auto_check_fold').hide();
            $('.jpoker_auto_check_call').hide();
            ...
            //
            // hand strength / hand odds (handmaker)
            //
            ...
            var hand_odds_element = $('#hand_odds' + id).html(jpoker.plugins.playerSelf.templates.hand_odds.supplant({
                        label: _("Hand Maker:")
                    })).hide();

            var timebank_element = $('#timebank' + id).html(jpoker.plugins.playerSelf.templates.timebank.supplant({
                        label: _("Time Bank:")
                    })).hide();						
        ...
        rebuy_options: { width: '370', height: '200', minHeight: '300px', dialogClass: "rebuy-dialog" , autoOpen: false, resizable: false },
        ...
        rebuy: function(url, game_id, serial, id) {
            ...
            if(!player || !server.userInfo.money) {
                return false;
            }

            var table = jpoker.getTable(url, game_id);

            // player should be able to rebuy when game is in session, so long as player has folded
            if(table.state != 'end' && player.in_game) {
                jpoker.dialog("You are not able to rebuy chips while still involved in a hand.");
                return false;
            }

            if (player.money >= table.buyIn.max) {
                jpoker.dialog("You cannot rebuy until you have less than this table's maximum buy-in.");
                return false;
            }
            ...
            var message = "";
            // 20121217 SCL Following lines are causing error / table destroy
            //  when user refreshes table while buy-in dialog is being displayed.
            //  Adding server.userInfo.money check a few lines above to prevent full crash.
            //  Still kind of broken, though.
            var total_chips = server.userInfo.money.X1[0];
            var live_chips = server.userInfo.money.X1[2];
            var regular_chips = (total_chips - live_chips);

            var in_buyin_scenario = player.state == 'buyin' && !player.buy_in_payed;
            var rathole_data = server.userInfo.rathole;
            var in_rathole_scenario = rathole_data != "";

            var table_bankroll_min = 0;
            if (in_buyin_scenario) {
                var bet_struct = table.betting_structure;
                var big_blind = parseInt(bet_struct.split('-')[1], 10);
                if (bet_struct.indexOf('no-limit') > -1 || bet_struct.indexOf('pot-limit') > 1)
                {
                    // No bankroll min on 1-2 tables
                    if (big_blind > 2) {
                        table_bankroll_min = 500 * big_blind;
                    }
                }
                else // fixed limit holdem
                {
                    // No bankroll min on 2-4 tables
                    if (big_blind > 4) 
                        table_bankroll_min = 300 * big_blind;
                }
            }
            ...
            var label;
            if (in_buyin_scenario) {
                packet_type = 'PacketPokerLiveAceBuyIn';
                label = _("buyin_btn.png");
            } else {
                packet_type = 'PacketPokerLiveAceRebuy';
                label = _("rebuy_btn.png");
                // rathole doesn't apply in rebuy
                // this also ensures the rathole message doesn't appear in dialog
                in_rathole_scenario = false;
            }

            // The buyin amount for this table is typically stored in table.buyIn.min.
            // However, if we are in rathole scenario, the buyin will be higher
            var table_buyin = in_rathole_scenario ? rathole_data.live_chips + rathole_data.real_chips : table.buyIn.min;

            // if this is a rebuy, we don't care about table.buyIn; we want the diff btw current chips and table.buyIn
            if ( ! in_buyin_scenario ) { // this is a rebuy
                // note that in a rebuy scenario, table_buyin is the max buyin
                table_buyin = table_buyin - player.money;
            }

            // can the player afford to buy in?
            // table.buyIn.bankroll has total chips (LC+RC) of this player
            // First check bankroll min.
            if (table.buyIn.bankroll < table_bankroll_min) {
                jpoker.dialog("You do not have enough chips. The bankroll minimum for this table is " + jpoker.chips.LONG(table_bankroll_min) + ". Please see the Gameplay FAQ for details.");
                return false;
            }
            // Now check the table buyin min.
            // TODO: SCL fix this for rebuys.
            // in a rebuy scenario, since table_buyin is storing max buyin, this dialog is sometimes being
            //  shown incorrectly, i.e. they could rebuy at some level, just not the max
            if (table.buyIn.bankroll < table_buyin) {
                jpoker.dialog("You do not have enough chips. The buy in minimum for this table is " + jpoker.chips.LONG(table_buyin) + ".");
                return false;
            }
            else { // it seems we have enough total chips
                // ...but in a rathole scenario, we must satisfy a certain amount of LCs, regardless of total chips
                if (in_rathole_scenario && (live_chips <  rathole_data.live_chips)) { 
                    jpoker.dialog("Sorry, you may not play at this table right now, as you have only " + jpoker.chips.LONG(live_chips) + " available LCs.<br/><br/>" + rathole_data.message);
                    return false;
                }

                // if we reach this point then we can use a loose mixture of RC/LC if needed
                if (regular_chips < table_buyin) { // dipping into live chips
                    lc_buyin = table_buyin - regular_chips;
                    rc_buyin = regular_chips;
                    message = "You are about to use " + jpoker.chips.LONG(rc_buyin) + " RCs and " + jpoker.chips.LONG(lc_buyin) + " LCs to buy into this game. You can always buy more RCs by clicking the Buy Chips button.";
                }
                else { // we have enough regular chips
                    rc_buyin = table_buyin;
                }
            }
            ...
            if(rebuy.size() === 0) {
                $('body').append('<div style=\'\' id=\'jpokerRebuy\' class=\'jpoker_jquery_ui\' title=\'' + _("Add chips") + '\' />');
                rebuy = $('#jpokerRebuy');
                rebuy.dialog(this.rebuy_options);
            }

            if (in_rathole_scenario) {
                var rathole_buyin = rathole_data.live_chips + rathole_data.real_chips;
                var min = rathole_buyin;
                var current = rathole_buyin;
                var max = rathole_buyin;
                rc_buyin = rathole_data.real_chips;
                lc_buyin = rathole_data.live_chips;
            }
            else {
                var min = 0;
                var current = table_buyin;
                var max = table_buyin;
            }

            rebuy.html(this.templates.rebuy.supplant({
                        'min': jpoker.chips.SHORT(min),
                        'max': jpoker.chips.SHORT(max),
                        'current': jpoker.chips.SHORT(current),
                        'title' : Math.floor(current),
                        'message' : in_rathole_scenario ? rathole_data.message : message,
                        'label': label,
                        'auto_sitin_label': _("Sit in"),
                        // following are the values displayed in dialog to player
                        'lc_buyin': lc_buyin, 
                        'rc_buyin': rc_buyin
            }));
            ...
            // if player is using LCs, show numbers in red to alert them to that fact
            if (lc_buyin > 0) {
                $('.jpoker_rebuy_live_btn span').css('color', 'red');
                $('.jpoker_rebuy_current').css('color', 'red');
            }
            ...
            $('.jpoker_rebuy_action', rebuy).click(function() {
                    ...
                    if(server) {
                        ...
                        var rc_amount = parseInt( $('.jpoker_rebuy_regular_btn span').html() );
                        var lc_amount = parseInt( $('.jpoker_rebuy_live_btn span').html() );
                        var amount = rc_amount + lc_amount;
                        if (amount == 0) { 
                         // nothing to do
                            ...
                            server.sendPacket({ 'type': packet_type,
                                        ...
                                        'amount': amount,
                                        'amount_lc': lc_amount,
                                        'rathole_lc': in_rathole_scenario ? amount - table.buyIn.min : 0
                                }, 
                                function() {
                                      ...
                                      // After the Buyin / Rebuy packet is sent, call getUserInfo so that chip amounts will
                                      //   be accurate next time we need them.
                                      server.getUserInfo();
                                }
                            );
                        } else {
            ...
            var slider_changed = function(event, ui) {
                // show the current value and the RC/LC breakdown
                var rc_value = Math.min(ui.value, regular_chips);
                var lc_value = ui.value - rc_value;

                var current = $('.jpoker_rebuy_current').html(ui.value);
                var reg = $('.jpoker_rebuy_regular_btn span').html(rc_value);
                var live = $('.jpoker_rebuy_live_btn span').html(lc_value);
                ...
                if (lc_value > 0) {
                  ...
                  current.css("color", "#222222");
                  live.css("color", "#222222");
                }
            ...
            $('.ui-slider-1', rebuy).slider({
                        ...
                        slide: slider_changed
            });

            // simplify buy-ins by having min == max == best 
            // therefore, eliminate slider
            if (in_buyin_scenario) {
                $('.ui-slider-1', rebuy).css('display', 'none');
                $('.jpoker_rebuy_current', rebuy).css('display', 'none');
                $('.jpoker_rebuy_min', rebuy).css('display', 'none');
                $('.jpoker_rebuy_max', rebuy).css('display', 'none');
            }
            else { // rebuy
                $('.jpoker_rebuy_message').hide();
            }

            // in rat hole scenario, hide unnecessary elements and add a No Thanks button
            if (rathole_data != "") {
                $('.jpoker_rebuy_current', rebuy).css('display', 'none');
            }
            
            // If there is a message being displayed, this is a special-case buyin, so add a No Thanks button
            /*
            if (in_rathole_scenario || message != "") {
                $('.ui-dialog-buttonpane', rebuy).append("<button class='jpoker_nothanks_action'>No Thanks</button>");
                $('.jpoker_nothanks_action').click(function() {
                    $('#jpokerRebuy').dialog('close'); 
                });
            } */
        ...
        handStart: function(player, id) {
            ...
            var hand_odds_element = $('#hand_odds' + id).hide();
            $('.jpoker_hand_odds_value', hand_odds_element).text('');

            var timebank_element = $('#timebank' + id).hide();
            $('.jpoker_timebank_value', timebank_element).text('');
			$('.jpoker_ptable_timebank_bar').hide();

			$('.slidechart').find(".background1").css('width',  '0px');
			$('.slidechart').find(".background2").css('width',  '0px');
			$('.slidechart').find(".background3").css('width',  '0px');
			$('.slidechart').find(".background4").css('width',  '0px');
			
			$('#seat' + player.seat + id).unbind('hover');
			
			$('input[name=auto_fold]')[0].checked = false;
			//DELTEME <- C
		
		
		function updateBars()
		{
			$(".background4").css("width", current_width+"%");
			current_width += 10;
			if(current_width > 100) current_width = 0; 
			setTimeout(function(){updateBars();}, 500);
		}
		//updateBars();
		/////////////
        ...
        handOdds: function(player, odds, id) {
            var hand_odds_element = $('#hand_odds' + id).show();
			odds = odds.replace("_", " ");
            $('.jpoker_hand_odds_value', hand_odds_element).html(odds);
			$('.jpoker_hand_odds_value').find("th").remove();
			$('.jpoker_hand_odds_value').find("td:nth-child(1)").css('white-space', 'nowrap');
			$('.jpoker_hand_odds_value').find("td:nth-child(2)").css('text-align','right');
			$('.jpoker_hand_odds_value').find("td:nth-child(2)").css('width','25px');
			
			if(isMobile.any())
			{
				$('.slidechart').hide();
				$('.jpoker_hand_odds_value').find("table").css('width','115px');
			} else {
			...
			probability1 = parseInt($('.jpoker_hand_odds_value').find("tr:nth-child(2) td:nth-child(2)").text());
			probability1 = 33.0 * probability1 / 100.0;
			//alert(probability1);
			setTimeout(function() {$('.slidechart').find(".background1").css('width',  '' + probability1 + 'px');},100);
			$('.slidechart').find(".background1").width('' + probability1 + 'px');
			
			probability2 = parseInt($('.jpoker_hand_odds_value').find("tr:nth-child(3) td:nth-child(2)").text());
			probability2 = 33.0 * probability2 / 100.0;
			//alert(probability2);
			setTimeout(function() {$('.slidechart').find(".background2").css('width',  '' + probability2 + 'px');},100);
			$('.slidechart').find(".background2").width('' + probability2 + 'px');
			
			probability3 = parseInt($('.jpoker_hand_odds_value').find("tr:nth-child(4) td:nth-child(2)").text());
			probability3 = 33.0 * probability3 / 100.0;
			//alert(probability3);
			setTimeout(function() {$('.slidechart').find(".background3").css('width',  '' + probability3 + 'px');},100);
			$('.slidechart').find(".background3").width('' + probability3 + 'px');
			
			probability4 = parseInt($('.jpoker_hand_odds_value').find("tr:nth-child(5) td:nth-child(2)").text());
			probability4 = 33.0 * probability4 / 100.0;
			//alert(probability4);
			setTimeout(function() {$('.slidechart').find(".background4").css('width',  '' + probability4 + 'px');},100);
			$('.slidechart').find(".background4").width('' + probability4 + 'px');
			
			/* setTimeout(function(){
				$('.slidechart').find(".background1").forceRedraw(true);
				$('.slidechart').find(".background2").forceRedraw(true);
				$('.slidechart').find(".background3").forceRedraw(true);
				$('.slidechart').find(".background4").forceRedraw(true);
			}, 100); */
			
            jpoker.plugins.playerSelf.callback.hand_odds.display_done(hand_odds_element);
			//},400);
        },

        timeBank: function(player, timebank_remainder, id) {
            var timebank_element = $('#timebank' + id).show();
			$('.jpoker_ptable_timebank_bar').show();
            if (timebank_remainder < 0) { // already depleted
                $('.jpoker_timebank_value', timebank_element).html(0);
                jpoker.plugins.playerSelf.callback.timebank.display_done(timebank_element);
                return;
            }

//            $('.jpoker_timebank_value', timebank_element).html(timebank_remainder);
				if (timebank_remainder >= 0 && timebank_remainder <= 9)
				{
	                $('.jpoker_timebank_value', timebank_element).html(':0' + timebank_remainder);
				} else {
					$('.jpoker_timebank_value', timebank_element).html(':' + timebank_remainder);
				}
			var pos;
			var count=0;
            timeBankIntervalId = setInterval(timer, 1000); // run every second (1000 millisecs)
			
			var url = player.url;
            var server = jpoker.getServer(url);

            function timer() {
                timebank_remainder -= 1;
				count += 1;
                if (timebank_remainder < 0) {
                    clearInterval(timeBankIntervalId);
                    jpoker.plugins.playerSelf.callback.timebank.display_done(timebank_element);
				...
				if (count <= 3 && count >= 1)
				{	
					jpoker.plugins.player.callback.sound.beep(server);
				} else if (timebank_remainder <= 4 && timebank_remainder >= 0)
				{
					jpoker.plugins.player.callback.sound.beep(server);
				}	
				if (timebank_remainder >= 0 && timebank_remainder <= 9)
				...
				$('.jpoker_ptable_timebank_redbar').css('background-position', '0 ' + (45-timebank_remainder) * 107 / 45 + 'px');

				//var timeout_element = $('#player_seat1_timeout' + id);
				var timeout_element = $('.jpoker_player_self .jpoker_timeout');
				$('.jpoker_timeout_progress', timeout_element).css('width',  66.5 * timebank_remainder / 45.0 + 'px');
			...
			function beeptimer() {
				...
				if (pos == true)
				{
					if (count >= 3)
					{
						clearInterval(timeBeepIntervalId);
					}
				} else {
					if (count >= 5)
					{
            ...
            name.click(function() {
                    ...
                    if(server) {
                        ...
                        if(player.money > jpoker.chips.epsilon) {
                            ...
                            // just click the rebuy button for him 
                            $('#rebuy' + id).click();
                        }
                    }
                });
        ...
        chips: function(player, id) {
            ...
            if (table.is_tourney) {
                ...
                // They used to programmatically click the Rebuy button here.
                // Don't do that anymore. With buyins we don't need refreshed 
                //  user info.
                if (typeof this.initialBuyin !== 'undefined') {
                    this.initialBuyin();
                }
            } else if (player.state != 'buyin' && player.broke) {
                // I'm not sure when/why this would ever happen. --SCL
                // Should prob change this to this.initialBuyin() too?
                //$('#rebuy' + id).click();
            }
        },
        ...
        inPosition: function(player, id) {
            ...
            var send = function(what) {
				...
				if (what == 'Fold')
				{
					$('#seat' + player.seat + id).hover(function(){
						$('#card_seat' + player.seat + id + ' .jpoker_card').css('opacity', '');
					  $('#card_seat' + player.seat + id + ' .jpoker_card').addClass('hover_card');
					},function(){
					  $('#card_seat' + player.seat + id + ' .jpoker_card').removeClass('hover_card');
					});
				}
            ...
            var auto_fold_input = $('input[name=auto_fold]', auto_action_element);
			...
			*/
			...
			//console.log("CHIT || "+bRaised);
			if(auto_fold_input.is(':checked')) {
				// if(betLimit.call > 0)
				if(bRaised)
				{
					send('Fold');
				} else if(betLimit.call > 0){
					send('Call');
				} else {
					send('Check');
				}
		   ...
		   /* $('input[type=checkbox]', auto_action_element).each(function() {
            ...
            //console.log("CHIT || "+ betLimit.allin);
            if(betLimit.allin > betLimit.call) {
                ...
                if(betLimit.max > betLimit.min) {
                    ...
                    raise_input.empty();
                    $('<input id=\'jpoker_raise_input_id\' class=\'jpoker_raise_input\' onClick=\'this.select();\' type=\'text\'>').appendTo(raise_input).val(betLimit.min);
                    
					raise_input.show();
					
					
					var mystr = $('.jpoker_table_info_blind').text();
					var myarr = mystr.split("-");
					var myvar = parseInt(myarr[1], 10);
                    $('.ui-slider-1', raise).slider({
                                ...
                                max: betLimit.max,
                                axis: 'horizontal',
                                step: myvar /*betLimit.step*/,
                                change: function(event, ui) {
                                  var current = $('.jpoker_raise_current', ui.element);
                                  current.html(jpoker.chips.SHORT(ui.value));
                                  current.attr('title', ui.value);
                                  if (! sliding) {
                                    $('.jpoker_raise_input', raise_input).val(ui.value);
									//$('.jpoker_raise_input', raise_input).focus(); /* Set Focus to RaiseInputTextBox */
									
									//$('.jpoker_raise_input', raise_input).val($('.jpoker_raise_input', raise_input).val());
                                  }
                                },
                    ...
                    var slider_raise_update = function() {
                      ...
                      if (isNaN(value)) {
                          ...
                          //J $('.jpoker_raise_input', raise_input).val(value);
						  $('.jpoker_raise_input', raise_input).val('');
                      } else {
                          $('.ui-slider-1', raise).slider('value', value);
                      }
                      sliding = false;
                    }
                    ...
                    click = function() {
                        ...
                        if(server) {
                            ...
                            if (!isNaN(amount)) {
								...
								var betClampSwap = -1;
								var betSideSwap = 0;
								$('.jpoker_player_dealt').each(function(){
									var nMoney = parseFloat(($('.jpoker_chips_amount', this).html()).replace(",", ""), 10);
									var nBet = parseFloat(($('.jpoker_bet .jpoker_chips_amount span', this).html()).replace(",", ""), 10);
									
									if(!isNaN(nMoney))
									{
										//if(!isNaN(nBet)) nMoney += nBet;
										var clamp_compare = nMoney + nBet;
										if(betClampSwap > nMoney) 
										{
											if(betClamp < nMoney) {
												betClamp = nMoney;
												betSide = nBet;
								...
								var clamp = (betSide + betClamp) - ( betSideSwap);
								if(amount > betClamp)
									amount = Math.min(amount, clamp);
                    ...
                    $('#allin' + id).unbind('click').click(function() {
                            ...
                            if(server) {
                                server.sendPacket({ 'type': 'PacketPokerRaise',
                                            'serial': serial,
                                            'game_id': game_id,
                                            'amount': Math.round(betLimit.allin)
                                            });
                            }
                        }).show();
                    if(betLimit.allin > betLimit.pot) {
                       $('#pot' + id).unbind('click').click(function() {
                            $(".jpoker_raise_input", raise_input).val(Math.floor(betLimit.pot));
                            slider_raise_update();
                        }).show();
					} else {
						$('#pot' + id).unbind('click').click(function() {
                            $(".jpoker_raise_input", raise_input).val(Math.floor(betLimit.allin));
                            slider_raise_update();
                        }).show();
					}
					if (betLimit.allin > Math.floor(betLimit.pot * 0.5))
					{
						$('#halfpot' + id).unbind('click').click(function() {
                            $(".jpoker_raise_input", raise_input).val(Math.floor(betLimit.pot*0.5));
                            slider_raise_update();
                        }).show();
					} else {
						$('#halfpot' + id).unbind('click').click(function() {
                    ...
                    if (betLimit.allin > Math.floor(betLimit.pot * 0.75)){
                       $('#threequarterpot' + id).unbind('click').click(function() {
                            $(".jpoker_raise_input", raise_input).val(Math.floor(betLimit.pot*0.75));
                            slider_raise_update();
                        }).show();
					} else {
						$('#threequarterpot' + id).unbind('click').click(function() {
                    ...
                    click = function() {
                        $(this).unbind('click');
                        var server = jpoker.getServer(url);
                        var amount = Math.min(betLimit.min, betLimit.allin);
                        if(server) {
                            server.sendPacket({ 'type': 'PacketPokerRaise',
                                        'serial': serial,
                                        'game_id': game_id,
				...
				/*J */
				$('.jpoker_raise_input', raise_input).unbind('keypress').keypress(function(e) {
					if(e.which == 13) {
						click();
					}
				});
        ...
        names: [ 'fold', 'call', 'check', 'raise', 'raise_range', 'raise_input', 'allin', 'pot', 'halfpot', 'threequarterpot' ], 
        ...
        templates: {
            rebuy: '<div style=\'float: left; font-size: 11px; font-weight: bold; position: relative; left: 42px; \' class=\'jpoker_rebuy_regular\'>REGULAR CHIPS</div>' + 
				'<div style=\'position: relative; left: -65px; float: right; font-size: 11px; font-weight: bold; \' class=\'jpoker_rebuy_live\'>LIVE CHIPS</div>' + 
				'<div style=\'clear: both; float: left; width: 113px; height: 23px; left: 36px; position: relative; margin-top: 10px; \' class=\'jpoker_rebuy_regular_btn\'><span style=\'font-size: 12px; font-weight: bold; margin-top: 4px; display: block; text-align: center;\'>{rc_buyin}</span></div>' + 
				'<div style=\'margin-top: 10px; position: relative; left: 65px; float: left; width: 113px; height: 23px;\' class=\'jpoker_rebuy_live_btn\'><span style=\'font-size: 12px; font-weight: bold; margin-top: 4px; display: block; text-align: center;\'>{lc_buyin}</span></div>' + 
				'<div class=\'jpoker_rebuy_bound jpoker_rebuy_min\'>{min}</div>' + 
				'<div style=\'border-bottom-right-radius: 6px; border-bottom-left-radius: 6px; border-top-right-radius: 6px; border-top-left-radius: 6px;\' class=\'ui-slider-1 ui-slider-gradient\'><div class=\'ui-slider-handle\' style=\'border-radius: 0; height: 1.8em; width: 1.8em; top: -0.35em; \' ></div></div>' + 
				'<div class=\'jpoker_rebuy_current\' title=\'{title}\'>{current}</div>' + 
				'<div style=\'color: #FF0000; text-align: center; width: 100%; font-size: 11px;\' class=\'jpoker_rebuy_message\'><p>{message}</p><!-- <p>You\'re using Live Chips. If you\'d like to purchase Regular Chips</p><p>instead, please click the <span style=\'font-weight: bold;\'>Buy Chips</span> button below.</p> --> </div>' + 
				'<div class=\'jpoker_rebuy_bound jpoker_rebuy_max\'>{max}</div>' + 
				'<div class=\'ui-dialog-buttonpane\'><button class=\'jpoker_rebuy_action\' style=\'background-image: url(\"/site_media/static/css/images/{label}\");\'>&nbsp; </button><button class=\'jpoker_buy_chips\' onclick=\'javascript:window.open(\"/tables/reload/\")\'>&nbsp;</button></div>' + 
				'<div class=\'jpoker_auto_sitin\'><input name=\'jpoker_auto_sitin\' type=\'checkbox\'></input><label for=\'jpoker_auto_sitin\'>{auto_sitin_label}</label></div>',
            auto_action: '<div class=\'jpoker_auto_check_fold jpoker_auto_action\' style=\'position: relative;text-align:right;margin-right:53px;margin-bottom:8px;\'><label for=\'auto_check_fold{id}\' style=\'font-size: 13px; width: 66px;\'>{auto_check_fold_label}</label><input type=\'checkbox\' name=\'auto_check_fold\' id=\'auto_check_fold{id}\' /></div><div class=\'jpoker_auto_check jpoker_auto_action\' style=\' position: relative;height:20px;text-align:right; margin-right:68px; margin-bottom:8px;\'><label for=\'auto_check{id}\' style=\'font-size: 13px; width: 33px;\'>{auto_check_label}</label><input type=\'checkbox\' name=\'auto_check\' id=\'auto_check{id}\' /></div><div class=\'jpoker_auto_check_call jpoker_auto_action\' style=\' position: relative; text-align:right;margin-right:53px;margin-bottom:8px;\'><label for=\'auto_check_call{id}\' style=\'font-size: 13px; width: 150px;\'>{auto_check_call_label}</label><input type=\'checkbox\' name=\'auto_check_call\' id=\'auto_check_call{id}\' /></div><div class=\'jpoker_auto_raise jpoker_auto_action\' style=\'position: relative;text-align:right;display:none!important; \'><label for=\'auto_raise{id}\' style=\'width: 32px; font-size: 13px; position:absolute!important;display:none!important; \'>{auto_raise_label}</label><input type=\'checkbox\' name=\'auto_raise\' id=\'auto_raise{id}\' style=\'display:none;\' /></div><div class=\'jpoker_auto_call jpoker_auto_action\' style=\' position: relative; text-align:right;margin-right:53px;margin-bottom:8px;\'><label for=\'auto_call{id}\' style=\'width: 39px; font-size: 13px; \'>{auto_call_label} <span class=\'jpoker_call_amount\'></span></label><input type=\'checkbox\' name=\'auto_call\' id=\'auto_call{id}\' /></div><div class=\'jpoker_auto_fold jpoker_auto_action\' style=\' position:absolute;top:-32px;left:-94px;\'><label for=\'auto_fold{id}\' style=\'width: 39px; font-size: 13px; \'>{auto_fold_label}</label><input type=\'checkbox\' name=\'auto_fold\' id=\'auto_fold{id}\' /></div>',
            hand_strength: '<span style=\'display: none;\' class=\'jpoker_hand_strength_label\'>{label}</span> <span style=\'display: none;\' class=\'jpoker_hand_strength_value\'></span>',
//J         hand_odds: '<span class=\'jpoker_hand_odds_label\'>{label}</span> <span class=\'jpoker_hand_odds_value\'></span>',
			hand_odds: '<span class=\'jpoker_hand_odds_value\'></span>',
//J         timebank: '<span class=\'jpoker_timebank_label\'>{label}</span> <span class=\'jpoker_timebank_value\'></span>',
            timebank: '<span class=\'jpoker_timebank_value\' style=\'color: white; font-size: 22px; font-weight: bold; \'></span>',
            action: '<div class=\'jpoker_button\'><a>{action}</a></div>',
            rebuy_button: '<div class=\'jpoker_rebuy\'><a style=\'border: medium none; width: 80px;\'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></div>',
...
//			sitin: '<div class=\'jpoker_sitin\'><a href=\'javascript://\'>{sitin}</a></div>'
            sitout: '<div class=\'jpoker_sitout\'></div>',
            sitin: '<div class=\'jpoker_sitin\'></div>'
        },
        ...
        callback: {
            sound: {
				...
				beep: function(server) {
					if(server.preferences.sound) {
                        $('#jpokerSound').html('<' + jpoker.sound + ' src=\'' + jpoker.sound_directory + 'player_timealert.swf\' />');
                    }
				},
            },
            ...
            hand_odds: { display_done : function() { } },

            timebank: { display_done : function() { } },
...
//            button: '<div class=\'jpoker_options\'><a href=\'javascript://\'>{options_label}</a></div>',
            button: '<div class=\'jpoker_options\'><a style=\'border: medium none; width: 80px;\'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a></div>',
    ...
    jpoker.plugins.muck = {
        ...
        AUTO_MUCK_LOSE: 2,
        templates : {
            muck_accept: '<div class=\'jpoker_muck jpoker_muck_accept\'><a>{muck_accept_label}</a></div>',
            muck_deny: '<div class=\'jpoker_muck jpoker_muck_deny\'><a>{muck_deny_label}</a></div>',
        ...
        muckRequest: function(server, packet, id) {
            if ($.inArray(server.serial, packet.muckable_serials) != -1) {
                // Instead of displaying show / muck buttons, just show.
                //$('#muck_accept' + id).show();
                //$('#muck_deny' + id).show();
                $('#muck_deny' + id).click();
            }
	...
	jpoker.plugins.checkboxes = {
		templates : {
			chkbox_fold: '<input type=\'checkbox\' name=\'pokercheck\' value=\'\' style=\'width: 0px;\' >{chkbox_fold_label}' ,
			chkbox_hand: '<input type=\'checkbox\' name=\'pokercheck\' value=\'\' style=\'width: 0px;\' checked=\'checked\'>{chkbox_hand_label}' ,
			chkbox_blind: '<input type=\'checkbox\' name=\'pokercheck\' value=\'\' style=\'width: 0px;\'>{chkbox_blind_label}' ,
		}
	};
	...
	// handmaker
	//
	jpoker.plugins.handmaker = {
		templates : {
			handmakerbg: '<div style=\'background-color: white; width: 119px; height: 88px; margin: 18px auto;\' >' + 
				'<div class=\'probchart\' style=\'width: 70%; height: 100%; float: left; \'></div><div class=\'slidechart\' style=\'width: 29%; height: 100%; float: left;\'><div style=\'width: 100%; height: 100%;\'><div style=\'width: 35px; height: 17px; margin-left: 0px; margin-top: 3px; \'><div class=\'background1\' style=\'width: 0%; height: 100%;\'></div></div><div style=\'width: 35px; height: 17px; margin-left: 0px; margin-top: 4px; \'><div class=\'background2\' style=\'width: 0%; height: 100%;\'></div></div><div style=\'width: 35px; height: 17px; margin-left: 0px; margin-top: 4px;\'><div class=\'background3\' style=\'width: 0%; height: 100%;\'></div></div><div style=\'width: 35px; height: 17px; margin-left: 0px; margin-top: 4px;\'><div class=\'background4\' style=\'width: 0%; height: 100%;\'></div></div></div></div></div>'
/*				'<input type=\'text\' value=\'{full_house}\' style=\'text-align: left; width: 75%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><input type=\'text\' value=\'5%\' style=\'text-align: right; width: 25%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><br/>' + 
				'<input type=\'text\' value=\'{flush}\' style=\'text-align: left; width: 75%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><input type=\'text\' value=\'10%\' style=\'text-align: right; width: 25%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><br/>' + 
				'<input type=\'text\' value=\'{straight}\' style=\'text-align: left; width: 75%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><input type=\'text\' value=\'50%\' style=\'text-align: right; width: 25%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><br/>' + 
				'<input type=\'text\' value=\'{kind_label}\' style=\'text-align: left; width: 75%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><input type=\'text\' value=\'0%\' style=\'text-align: right; width: 25%; font-size: 9px; font-weight: bold; font-family: arial; margin-top: 7px;\' /><br/></div></div>'*/
		}
	};
    ...
    jpoker.plugins.cards = {
        ...
        update_value: function(cards, prefix, id) {
			var seatnumber = prefix.substring(prefix.length - 1, prefix.length);
			var hole_element = $('#player_seat' + seatnumber + '_hole' + id);
			var dealt_element = $('#seat' + seatnumber + id);
			var flag = false;
			...
			if (flag == true)
			{
				hole_element.css('opacity', '0.0');
				//dealt_element.removeClass('jpoker_player_dealt');
			}
        },
    ...
    jpoker.plugins.chips = {
        ...
        update: function(chips, total, id) {
            var element = $(id);
			var nchips = chips;
			var chipArray = new Array(1, 5, 25, 100, 500, 1000, 5000, 25000, 100000, 500000, 1000000, 5000000);
			var nDivision = 0;
			var nCount = 0;
			var tCount = 0;
			var nTags = '';
			...
			var sum = 0;
            if(chips > 0) {
				if (element.hasClass('jpoker_bet'))
				{
					for (i = chipArray.length - 1; i >= 0 ; i-- )
					{
						if (chips >= chipArray[i])
						{
							nCount = nCount + 1;
							chips = chips % chipArray[i];
						}
					}
					chips = nchips;
					tCount = nCount;
					nCount = 0;
					if (element.hasClass('jpoker_ptable_player_seat4_bet') || element.hasClass('jpoker_ptable_player_seat5_bet'))
					{
						for (i = chipArray.length - 1; i >= 0 ; i-- )
						{
							if (chips >= chipArray[i])
							{
								nCount = nCount + 1;
								nDivision = parseInt(chips / chipArray[i]);
								nTags = nTags + '<div class=\'jpoker_chips_' + chipArray[i] + '_' + nDivision + ' jpoker_chips_' + nCount + '_position\'></div>'
								chips = chips % chipArray[i];
							}
						}
					} else if (element.hasClass('jpoker_ptable_player_seat0_bet') || element.hasClass('jpoker_ptable_player_seat8_bet'))
					{
						var tmp = 0;
						for (i = chipArray.length - 1; i >= 0 ; i-- )
						{
							if (chips >= chipArray[i])
							{
								...
								nDivision = parseInt(chips / chipArray[i]);
								tmp = tCount - nCount + 1;
								nTags = '<div class=\'jpoker_chips_' + chipArray[i] + '_' + nDivision + ' jpoker_chips_' + tmp + '_position\'></div>' + nTags;
								chips = chips % chipArray[i];
							}
						}
					} else if (element.hasClass('jpoker_ptable_player_seat1_bet') || element.hasClass('jpoker_ptable_player_seat2_bet') || element.hasClass('jpoker_ptable_player_seat3_bet') || element.hasClass('jpoker_ptable_player_seat6_bet') || element.hasClass('jpoker_ptable_player_seat7_bet'))
					{
						var left = 0;
						var top = 0;
						var left1 = 0;
						var top1 = 0;
						if (element.hasClass('jpoker_ptable_player_seat1_bet'))
						{
							left1 = 19;
							top1 = -15;
						}
						if (element.hasClass('jpoker_ptable_player_seat2_bet'))
						{
							left1 = 1;
							top1 = -6;
						}
						if (element.hasClass('jpoker_ptable_player_seat3_bet'))
						{
							left1 = -29;
							top1 = -15;
						}
						if (element.hasClass('jpoker_ptable_player_seat6_bet'))
						{
							left1 = -24;
							top1 = -15;
						}
						if (element.hasClass('jpoker_ptable_player_seat7_bet'))
						{
							left1 = -24;
							top1 = -15;
						}
						...
						for (i = chipArray.length - 1; i >= 0 ; i-- )
						{
							if (chips >= chipArray[i])
							{
								nCount = nCount + 1;
								nDivision = parseInt(chips / chipArray[i]);
								top = top1 + (1 - nCount) * 40;
								if (nCount > 6)
								{
									top = top + 25;
								}
								left = left1 + sPos - 28 * ( (nCount%6) - 1);
								nTags = nTags + '<div class=\'jpoker_chips_' + chipArray[i] + '_' + nDivision + ' jpoker_chips_' + nCount + '_position\' style=\'position: relative; top: ' + top  + 'px; left: ' + left   + 'px; \'></div>';
								chips = chips % chipArray[i];
							}
						}
					}
					...
					nCount = nCount + 1;
					if (element.hasClass('jpoker_ptable_player_seat0_bet') || element.hasClass('jpoker_ptable_player_seat8_bet'))
					{
						amountTag = '<div style=\'padding-top: 14px; \' class=\'jpoker_chips_amount jpoker_chips_' + nCount + '_position\'><span style=\'padding-left: 10px; float: left;\'></span></div>';
					} else if (element.hasClass('jpoker_ptable_player_seat4_bet') || element.hasClass('jpoker_ptable_player_seat5_bet'))
					{
						amountTag = '<div style=\'padding-top: 14px; \' class=\'jpoker_chips_amount jpoker_chips_' + nCount + '_position\'><span style=\'padding-right: 10px; float: right;\'></span></div>';
					} else if (element.hasClass('jpoker_ptable_player_seat1_bet') || element.hasClass('jpoker_ptable_player_seat2_bet') || element.hasClass('jpoker_ptable_player_seat3_bet') || element.hasClass('jpoker_ptable_player_seat6_bet') || element.hasClass('jpoker_ptable_player_seat7_bet'))
					{
						if (nCount % 2 == 0)
						{
					...
					nTags = nTags + amountTag;
					element.html(nTags); 
					element.show();
	                $('.jpoker_chips_amount span', element).text(jpoker.chips.SHORT(nchips));

					/* Update after every bet */
					/*for (i = 0; i < 9; i++)
					{
						if ( $('.jpoker_ptable_player_seat' + i + '_bet').css('display') != 'none' )
						{
							sum += parseInt($('.jpoker_ptable_player_seat' + i + '_bet ' + '.jpoker_chips_amount span').text());
						}
					}*/
				} else if (element.hasClass('jpoker_money'))
				{
	                element.show();
	                $('.jpoker_chips_amount', element).text(jpoker.chips.SHORT(nchips));
				}
				if (element.hasClass('jpoker_pot'))
				{	
					for (i = chipArray.length - 1; i >= 0 ; i-- )
					{
						if (chips >= chipArray[i])
					...
					for (i = chipArray.length - 1; i >= 0 ; i-- )
					{
						if (chips >= chipArray[i])
						{
							...
							nDivision = parseInt(chips / chipArray[i]);
							top = (1 - nCount) * 40;
							left = sPos - 28 * (nCount - 1);
							nTags = nTags + '<div class=\'jpoker_chips_' + chipArray[i] + '_' + nDivision + ' jpoker_chips_' + nCount + '_position\' style=\'position: relative; top: ' + top  + 'px; left: ' + left   + 'px; \'></div>';
							chips = chips % chipArray[i];
						}
					}
					nCount = nCount + 1;
					nTags = nTags + '<div style=\'padding-top: 14px; position: absolute; top: 24px; left: -24px; height: 15px; width: 86px; text-align: center; \' class=\'jpoker_chips_amount jpoker_chips_' + nCount + '_position\'><span></span></div>';
					if (element.hasClass('jpoker_pot0'))
					{
						nTags = nTags + '<div style=\'padding-top: 14px; position: absolute; top: -90px; left: -24px; height: 15px; width: 86px; text-align: center; \' class=\'jpoker_chips_amount1\'><span></span></div>';
					}
					element.html(nTags); 
					element.show();
	                /*$('.jpoker_chips_amount span', element).text('Pot: ' + jpoker.chips.SHORT(nchips));*/
					$('.jpoker_chips_amount span', element).text(jpoker.chips.SHORT(nchips));
					$('.jpoker_chips_amount1 span', element).text('Pot: ' + jpoker.chips.SHORT(total));
				}
                element.attr('title', jpoker.chips.LONG(nchips));
    ...
    jpoker.plugins.raise = {
        ...
        getHTML: function(betLimit, player) {
            ...
            return t.supplant({raise_label: label,
                                                ...
                                                raise_current_title: Math.floor(betLimit.min),
    ...
    jpoker.plugins.playerLookup.templates = {
        ...
        challenge: '<div class=\'jpoker_player_lookup_challenge\'><a>{label}</a></div>'
    };
    ...
    jpoker.plugins.cashier.getHTML = function(packet) {
        ...
        $.each(packet.money, function(currency_serial, money) {
                html.push(t.currencies.rows.supplant({currency_serial: currency_serial.substr(1),
                                currency_amount: money[0]=money[0],
                                currency_ingame: money[1]=money[1],
    ...
    jpoker.preferences.prototype = {
        // These muck preferences are now superceded by the players muck_preferences.
        auto_muck_win: false,

  ...
  var NOP_FUNC = function(){},
    ...
    ajaxWithRetry = function(settings){
      ...
      settings.error = function(xhr_obj, textStatus, errorThrown){
        var can_retry;
		console.log(["ajax error", xhr_obj, textStatus, errorThrown]);
        ...
        if (!can_retry || failures >= opts.attempts) {
          ...
          $('.ajax-retry').remove();
          $("#main").append("<div class='ajax-retry'>Lost connection...</div>");
          ...
          if (failures > 2) {
            $('.ajax-retry').remove();
            $("#main").append("<div class='ajax-retry'>Connecting"+ Array(failures+1).join('.') + "</div>");
          }
